...
void ADSKEmployeeReactor::openedForModify (const AcDbObject* pObj) {
	if ( DocVars.docData ().m_doRepositioning || !DocVars.docData ().m_editCommand ) {
		return ;
	}

	if ( isObjectAnEmployeeBlock (pObj) ) {
		DocVars.docData ().m_changedObjects.append (pObj->objectId ()) ;
		DocVars.docData ().m_employeePositions.append (AcDbBlockReference::cast (pObj)->position ()) ;
	}
}
